...
def clear_cuda_cache_for_all_gpus():
    # Store the current device context before switching
    ...
    for gpu_idx in range(num_gpus):
        # Set the current device context to the GPU
        torch.cuda.set_device(gpu_idx)

        # Attempt to clear the cache for the current device
        torch.cuda.empty_cache()

        # Optionally, perform additional cleanup tasks here

    # Switch back to CPU or another GPU if necessary
...
class SubstringEngine:
        ...
        for idx in range(len(tokenized_candidates)-1, -1, -1):
            ...
            if (torch.equal(candidate[:position], parent.token_sequence) and 
                ...
                if new_node.depth < max_depth:
                    self._expand_tree(new_node, tokenized_candidates_to_expand, max_depth, position + 1, special_ids)
            else:
    ...
    def _candidate_sequences(self, context, max_token_length, prompt=''):
        ...
        if self.is_fast:
            ...
            for i in range(len(context)):
                for j in range(i+1, min(len(context)+1, i+1+restriction)):
    ...
    async def _fast_logprobs(self, common_part, tokenized_candidates, k):   
        ...
        for candidate in tokenized_candidates:
            candidate_length = torch.sum(candidate != self.tokenizer.pad_token_id, dim=-1)
        ...
        for result in results:
            ...
            for token_id, log_prob in zip(candidate, result.prompt_logprobs[-len(candidate):]):
                if self.mode:
                    print(f"{self.tokenizer.decode(token_id)} : {log_prob[token_id].logprob}")
    ...
    async def _compute_logprob(self, common_part, nodes):
        ...
        for input_prompt in inputs_map:
            ...
            tokens_prompt: TokensPrompt = {
                'prompt_token_ids': list(input_prompt),
                'multi_modal_data': None  # If you have multi-modal data, provide it here
            }
        ...
        if self.mode:
            ...
            print(len(nodes_map))
            for node in nodes_map:
                print(f"'{self.tokenizer.decode(node.token_sequence)}'; '{node.cum_log_probability}'; '{node.depth}'")
    ...
    async def substring(self, prompt, context, k, max_substring_length, return_full_text=False):
        ...
        # print_torch_memory_usage()
        # print()
        chosed_options = await self._fast_logprobs(common_part_encoded, tokenized_s_cand, k)

        del common_part_encoded
        del tokenized_s_cand
        del substring_candidates
        ...
        for option in chosed_options:
            substirng_choice = self.tokenizer.decode(option, skip_special_tokens=True)
            if return_full_text: 
                    res_text.append(common_part + substirng_choice)
            else:
                res_text.append(substirng_choice[len(last_part):])
...
class GuidanceBeta:
    ...
    async def substring(self, input_text, context:str, k=1, max_substring_length=35):
        substring_engine = SubstringEngine(self.llm, self.tokenizer, mode=self.mode, is_fast=self.is_fast)
        # return res_text, working_list, initial_root
        result = await substring_engine.substring(input_text, context, k, max_substring_length)
        del substring_engine
        return result[0]
...
async def generate(request: Request) -> Response:
    ...
    if substring is None:
        ...
        return JSONResponse(content=[output.text for output in final_output.outputs][0])
    else:
        try:
