...
class SubstringEngine:
    ...
    def _build_tree(self, tokenized_context: List[torch.Tensor]) -> Tuple[Node, torch.Tensor]:
        ...
        print(f"{time.time() - s} sec: expand tree recursive call")
    ...
    async def _compute_logprob(self, common_part, nodes):
        ...
        async for item in results:
            if self.mode:
                print("all async generations are done")
        ...
        for node in node_output_map:
            ...
            while parent_tmp.cum_log_probability is None:
                ...
                parent_logprobs = log_probs_mapping.get(tuple(key), None)
                if parent_logprobs is None:

                    request_id = random_uuid()
                    res = engine.generate(request_id=request_id,
                                        prompt=None,
                                        prompt_token_ids=key,
                                        sampling_params=sampling_params)
                        
                    async for item in res:
                        if self.mode:
                            print("parent async generation is done to get logprobs")
                    
                    parent_logprob = log_probs_mapping[tuple(key)][parent_tmp.token_id]
                    parents_log_probs.append(parent_logprob)
                else:
                    parents_log_probs.append(parent_logprobs[parent_tmp.token_id])
    ...
    async def substring(self, prompt, context, k, max_substring_length, return_full_text=False):
        ...
        tokenized_s_cand = self.tokenizer(substring_candidates,
                                     ...
                                     padding=True,
                                     add_special_tokens=False)['input_ids']
        print(f"{time.time() - s} tokenization")
