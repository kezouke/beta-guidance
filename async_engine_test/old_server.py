...
class SubstringEngine:
    ...
    def _expand_tree(self, parent: Node,
        ...
        for idx, candidate in enumerate(tokenized_candidates):
            ...
            if position < len(candidate):
                ...
                if (torch.equal(candidate[:position], parent.token_sequence) and 
                    ...
                    if new_node.depth < height:
                        self._expand_tree(new_node, tokenized_candidates_to_expand, height, position + 1, special_ids)
                else:
                    tokenized_candidates_to_expand.pop(idx - removed_counter)
                    removed_counter += 1
            else:
                tokenized_candidates_to_expand.pop(idx - removed_counter)
                removed_counter += 1
    ...
    async def _compute_logprob(self, common_part, nodes):
        ...
        for idx in node_output_map:
            ...
            if self.mode:
                print("children: ")
                print(node.token_id)
                print(output.prompt_logprobs[-1][node.token_id])
